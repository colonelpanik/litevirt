...
Five NetBox failures are silent to everything else litevirt reports, so they are
...
leader lease, so the cluster has exactly one writer. The other three are
...
own host name, and a node that agrees never clears a peer's finding. All five are
**warning** severity, so they show as DEGRADED and never as CRITICAL, and none
gates admission.
