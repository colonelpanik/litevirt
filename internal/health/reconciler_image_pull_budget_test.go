...
func TestReconcilePass_ABackingImagePullOutlivesTheWalkBudget(t *testing.T) {
	...
	// row is back to pending with a reason, so the next pass picks it up.
	vm, err := corrosion.GetVM(ctx, db, "vm-pull")
	if err != nil || vm == nil {
		t.Fatalf("GetVM: %v", err)
	}
	if vm.State != "pending" {
		t.Fatalf("state = %q after the walk moved on, want pending (detail %q)", vm.State, vm.StateDetail)
	}
