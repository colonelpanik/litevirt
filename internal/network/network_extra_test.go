...
func TestFlushChain(t *testing.T) {
	...
	for _, call := range calls {
		if call[0] != "nft" || call[1] != "flush" || call[2] != "chain" {
			t.Errorf("expected nft flush chain, got %v", call)
		}
	}
}

func TestNftRuleArgs_DefaultAction(t *testing.T) {
	rule := ACLRule{
		Direction: "ingress",
		Proto:     "tcp",
		PortRange: "22",
		Action:    "", // should default to "accept"
	}
	args := nftRuleArgs("tap8", rule)
	last := args[len(args)-1]
	if last != "accept" {
		t.Errorf("default action should be accept, got %q", last)
	}
}

func TestNftRuleArgs_UDP_Egress_CIDR(t *testing.T) {
	rule := ACLRule{
		Direction: "egress",
		Proto:     "udp",
		PortRange: "53",
	...
	args := nftRuleArgs("tap9", rule)
	checkContains(t, args, "oifname")
	checkContains(t, args, "udp")
	checkContains(t, args, "sport")
	checkContains(t, args, "53")
	checkContains(t, args, "daddr")
	checkContains(t, args, "8.8.8.0/24")
	checkContains(t, args, "drop")
}

func TestNftRuleArgs_AllProto_WithCIDR_Egress(t *testing.T) {
	rule := ACLRule{
		Direction: "egress",
		Proto:     "all",
		CIDR:      "10.0.0.0/8",
	...
	args := nftRuleArgs("tap10", rule)
	checkContains(t, args, "oifname")
	checkContains(t, args, "daddr")
	checkContains(t, args, "10.0.0.0/8")
}

func TestNftRuleArgs_ICMP_WithCIDR_Egress(t *testing.T) {
	rule := ACLRule{
		Direction: "egress",
		Proto:     "icmp",
		CIDR:      "192.168.0.0/16",
	...
	args := nftRuleArgs("tap11", rule)
	checkContains(t, args, "oifname")
	checkContains(t, args, "protocol")
	checkContains(t, args, "icmp")
	checkContains(t, args, "daddr")
	checkContains(t, args, "192.168.0.0/16")
}
